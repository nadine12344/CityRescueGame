...
public class Simulator implements WorldListener {
	...
	public ResidentialBuilding getBuilding(int x,int y) {
		for(int i=0;i<buildings.size();i++) {
	...
	public Unit getUnitLoc(int x,int y) {
		for(int i=0;i<emergencyUnits.size();i++) {
			if(emergencyUnits.get(i).getLocation().getX()==x && emergencyUnits.get(i).getLocation().getY()==y)
	...
	public Citizen getCitizen(String s, ArrayList<Citizen> citizens2) {
		for(int i=0;i<citizens2.size();i++) {
			if(citizens2.get(i).getName().equals(s))
				return citizens2.get(i);
		}
		return null;
	}

	public Object[] getCitizens(int x, int y) {
		Object[] o = new Object [citizens.size()];
		for(int i=0;i<citizens.size();i++) {
			if(citizens.get(i).getLocation().getX()==x && citizens.get(i).getLocation().getY()==y)
				o[i] = citizens.get(i).getName();
		}
		return o;
	}

	public Citizen getCitizen(String string) {
		for(int i=0;i<citizens.size();i++) {
			if(citizens.get(i).getName().equals(string))
				return citizens.get(i);
		}
		return null;
	}
